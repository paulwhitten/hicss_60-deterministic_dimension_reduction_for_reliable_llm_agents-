...
analogy with \textcite{marr1982vision}'s 2.5-D sketch, reduces the likelihood
...
principle applicable beyond any single domain or benchmark. This is an
improvement contribution \parencite{gregorhevner2013}: reduce LLM errors on
deterministic output dimensions by offloading them to a provably correct
symbolic executor when those dimensions are computable in closed form.
BWIM and IGLU provide an artificial rather than naturalistic evaluation,
using benchmark tasks rather than deployed use.
...
guarantees that a correct symbolic executor strictly dominates the language
model on those dimensions for any positive error rate $\varepsilon > 0$. The only
...
We refer to these three categories as domain-law constraints,
procedural-knowledge constraints, and workflow-state constraints.
...
The BWIM grid is a 2.5-D domain in the sense of \textcite{marr1982vision}: the
...
\begin{align}
  ...
  \mathrm{EV}_{\mathrm{ask}} &= 10 - 5 = 5 \label{eq:ev_ask}
\end{align}
assuming the upstream architect, itself an LLM agent with access to the
target structure, answers correctly. Setting equal and solving yields
...
harness, is publicly available \parencite{bwim}. The NAECON paper
...
$n = 6$ respectively, on the identical frozen pipeline, and a
no-decomposition ablation of GPT-4o generating coordinates directly
($n = 8$). All prompt
...
collaborative building dataset \parencite{iglu2022}, the first 500 of the
3{,}365 (53\%) of 6{,}349 single-turn tasks whose targets contain no
floating blocks, compare a bare
coordinate-output prompt against the 2.5-D decomposed prompt using
GPT-4o-mini in a single run at temperature 0, with no enrichment rules, plan verifier, or
...
Nemotron-3-Super-120B-A12B-NVFP4 \parencite{nemotron2026} runs locally on
...
13 examples designed to exceed the model's Mamba-2 page size (8,320 tokens). Error attribution classifies all 1,920 GPT-4o-mini rounds as
...
all ablation effects except the plan verifier remain significant under
Holm-Bonferroni correction at $\alpha = 0.05$. Run-to-run variance reflects LLM
...
pp, $p = 0.36$, $n = 7$), consistent with enrichment rules preventing
most errors the verifier would otherwise target.
A reanalysis of the existing ablation runs decomposes this gap.
Reapplying the deterministic height rule to the model's own 3D outputs
recovers 19.2 pp to about 85\%, but the remaining 9.5 pp stays below the
full system, indicating that post-hoc correction remedies some but not all
of the loss and that removing the dimension from the schema carries
independent value, though this residual may also reflect that the 3D
condition reuses examples written for the 2D format.
...
In a controlled comparison, GPT-4o generating coordinates directly in the
ablation configuration reaches 79.4\% (mean over 8 runs, $\sigma = 0.55$ pp),
15.2 pp below GPT-4o-mini with the full pipeline.

A cursory cross-model check on the untuned pipeline via Azure AI Foundry
found DeepSeek-V3.2 at 95.6 to 96.2\% over six runs,
...
GPT-4o-mini. Assuming comparable token counts, relative cost is governed by
this per-token price ratio, though this may not hold since models emit
different token counts for the same input, especially reasoning models. A
160-round GPT-4o-mini run cost approximately
...
95\% CI $[0.050, 0.101]$). The improvement persists on a separate set of
500 additive-only gravity-compatible tasks that exclude removals, where F1
rises from 0.776 to
0.820 (paired $t(499) = 4.39$, $p < 0.001$). The effect is modest ($d = 0.26$), as expected, since only the
decomposition, not the enrichment rules, verifier, or clarification
handling, was transferred. This confirms the principle
...
experts). This demonstrates that the architecture transfers
...
exceeds the model's Mamba-2 page size (8,320 tokens), enabling cache hits that are not possible when the prompt falls
...
percentage points (95.6\%). The low-effort setting, the longer prompt, and
crossing the prefix cache page boundary act together in this result, so
isolating each factor's contribution would require a factorial design.
Tegrastats measurements show 71\,W mean board draw under full reasoning
and 75\,W under low-effort, dominated by the 108--119\,GB DRAM footprint
of the loaded model. Per-request energy is approximately 4.2\,kJ
(71\,W\,$\times$\,59.0\,s) and 1.5\,kJ (75\,W\,$\times$\,19.7\,s)
respectively, a 2.8$\times$ reduction.
...
state, classifying them as domain-law constraints, procedural-knowledge
constraints, or workflow-state constraints. Remove them from
the LLM output schema and compute them with a symbolic executor.
...
GPT-4o result may be understated.

The 15 enrichment rules were derived from BWIM failure modes, and their
coverage of other instruction distributions is untested.
...
higher token cost. The taxonomy of three categories, domain-law constraints,
procedural-knowledge constraints, and workflow-state constraints,
provides a structure for identifying decomposition
